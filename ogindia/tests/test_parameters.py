...
import tempfile
import pytest
from ogindia.parameters import Specifications, reform_warnings_errors
...
def test_reform_warnings_errors():
    user_mods = {'ogindia': {'frisch': 0.3}}

    ew = reform_warnings_errors(user_mods)
    assert len(ew['ogindia']['errors']) == 0
    assert len(ew['ogindia']['warnings']) == 0

    user_mods = {'ogindia': {'frisch': 0.1}}

    bad_ew = reform_warnings_errors(user_mods)
    assert len(bad_ew['ogindia']['errors']) > 0
    assert len(bad_ew['ogindia']['warnings']) == 0
