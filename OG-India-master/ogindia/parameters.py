...
# import ogindia
from ogindia.parametersbase import ParametersBase
from ogindia import elliptical_u_est
from ogindia import demographics
from ogindia import income
from ogindia import txfunc
from ogindia.utils import (BASELINE_DIR, TC_LAST_YEAR, rate_conversion,
                         safe_read_pickle)
# from ogindia import elliptical_u_est
...
class Specifications(ParametersBase):
    '''
    Inherits ParametersBase. Implements the PolicyBrain API for OG-India
    '''
    ...
    def __init__(self,
        ...
        # put OG-India version in parameters to save for reference
        self.ogindia_version = pkg_resources.get_distribution("ogindia").version
...
def reform_warnings_errors(user_mods):
    '''
    Generate warnings and errors for OG-India parameter specifications
    ...
    '''
    rtn_dict = {'ogindia': {'warnings': '', 'errors': ''}}

    # create Specifications object and implement reform
    specs = Specifications()
    specs._ignore_errors = True
    try:
        specs.update_specifications(user_mods['ogindia'], raise_errors=False)
        rtn_dict['ogindia']['warnings'] = specs.parameter_warnings
        rtn_dict['ogindia']['errors'] = specs.parameter_errors
    except ValueError as valerr_msg:
        rtn_dict['ogindia']['errors'] = valerr_msg.__str__()
    return rtn_dict
